 ...
 const priceInDollar = formatPrice(price);
  ...
  return (
    ...
    <div className='grid gap-y-6 gap-x-10 mt-4 md:grid-cols-2'>
      <div>
    <Image className='rounded w-full h-60 md:h-full object-cover' src={image} alt={name} width={4000} height={4000} quality={90} />
      </div>
    <div>
     ...
     <FavoriteToggleButton productId={id} />
      ...
      <h4>{company}</h4>
      <p className='mt-4 bg-muted inline-block rounded p-2'>{priceInDollar}</p>
      <p className='mt-6 text-base lg:text-lg text-muted-foreground'>{description}</p>
